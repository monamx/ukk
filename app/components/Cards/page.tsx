...
const Cards = () => {
  ...
  const [openedPopupFilePlus, setOpenedPopupFilePlus] = useState<string | null>(null);
  ...
  const handlePopup = (bukuId: string, type: string) => {
    if (type === 'message') {
      setOpenedPopupBook(prevBookId => (prevBookId === bukuId ? null : bukuId));
    } else if (type === 'file') {
      setOpenedPopupFilePlus(prevFilePlusId => (prevFilePlusId === bukuId ? null : bukuId));
    }
  };
  ...
  return (
    <>
              ...
              <div className="card-body">
                ...
                <div className="icon-container">
                  ...
                  <div className="icon-group">
                    ...
                    <span className="review-icon" onClick={() => handlePopup(item.bukuId, 'message')}><MessageSquare size={23} /></span>
                    <span className="review-icon" onClick={() => handlePopup(item.bukuId, 'file')}><FilePlus size={23} /></span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {listBuku.map((item) => (
        <div key={item.bukuId} className={`ulasan-popup ${openedPopupFilePlus === item.bukuId ? 'ulasan-show' : ''}`}>
          {openedPopupFilePlus === item.bukuId && (
            <>
              <div className="popup-content">
              {/* <span className="close" onClick={() => {setSubTag(true); setNewCategory(true);}}>&times;</span> */}
              <input
                type="text"
                placeholder="Masukkan nama kategori...${item.bukuId}"
                className="search-input"
                // value={NewCategory ? valueKategori : editCategory}
                // onChange={(e) => {
                //   if (NewCategory) {
                ...
                // }}
              />
              {/* <button onClick={handleCategoryAction}>{NewCategory ? 'Tambah Kategori' : 'Update Kategori'}</button> */}
              </div>
            </>
          )}
        </div>
